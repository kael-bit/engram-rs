...
衰减基于活动周期触发，系统空闲期间不衰减。

**指数衰减**遵循 [Ebbinghaus 遗忘曲线](https://en.wikipedia.org/wiki/Forgetting_curve)——先快后慢，长尾保留。记忆永远不会完全消失（下限 = 0.01），在精确查询下仍可被检索。当记忆被召回时会获得**激活增强**，让高频使用的知识自我强化。

| 类型 | 衰减速率 | 半衰期 | 适用场景 |
|------|---------|--------|---------|
| `episodic` | 最快 | ~35 轮 | 事件、经历等时效性内容 |
